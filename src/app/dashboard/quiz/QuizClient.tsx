...
export default function QuizClient({ topic, onBack }: { topic: string; onBack: () => void }) {
    ...
    const fetchQuiz = async () => {
      try {
        ...
        toast({
          ...
          description: 'Could not fetch quiz questions. Please try again.',
          variant: 'destructive',
        });
        onBack();
      } finally {
  ...
  const saveQuizAttempt = useCallback(async (finalResults: QuizResult[], finalQuiz: GenerateQuizOutput) => {
    if (!auth.currentUser) return;

    const score = finalResults.filter(r => r.isCorrect).length;
    const totalQuestions = finalQuiz.questions.length;

    const quizAttemptsCol = collection(
      firestore,
      'users',
      auth.currentUser.uid,
      'quizAttempts'
    );
    addDocumentNonBlocking(quizAttemptsCol, {
  ...
  const handleSubmitAnswer = () => {
    ...
    if (currentQuestionIndex === quiz.questions.length - 1) {
      saveQuizAttempt(updatedResults, quiz);
    }
  };
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center text-center p-8">
        ...
        <p className="mt-4 text-muted-foreground">Generating your {topic} quiz...</p>
      </div>
    );
  }
